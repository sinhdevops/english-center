						...
						{listArticles.map((item: any, i: number) => {
							...
							return (
								...
								>
									...
									{/* Content */}
									<div className="flex min-w-0 flex-1 flex-col justify-between p-4 sm:py-5 sm:pr-5 sm:pl-0">
										<div>
											<div className="mb-2 flex flex-wrap items-center gap-2">
												<span className="flex h-7 items-center gap-1 rounded-lg bg-[#E4E4E4] px-2.5 text-[13px] font-medium leading-5 text-[#4E4E4E]">
										...
										{isVideo ? (
											<button
												...
												className="bg-stem-blue mt-4 w-full rounded-lg px-6 py-2.5 text-xs font-black text-white transition-opacity hover:opacity-90 sm:w-fit sm:px-4 sm:py-1.5"
											>
												XEM VIDEO
											</button>
										) : (
											<Link href={`${detailBase}/${item.id}`} className="w-full sm:w-fit">
												<button
													type="button"
													className="bg-stem-blue mt-4 w-full rounded-lg px-6 py-2.5 text-xs font-black text-white transition-opacity hover:opacity-90 sm:px-4 sm:py-1.5"
												>
													XEM CHI TIẾT
												</button>
											</Link>
							...
							return (
								<div key={course.id || i} className={`${color.bg} ${color.border} rounded-2xl border p-5`}>
									...
									<Link href="/khoa-hoc/dang-ky">
										<button
											type="button"
											className={`${color.btn} rounded-lg border px-4 py-1.5 text-xs font-bold transition-colors`}
										>
											Đăng ký
										</button>
									</Link>
