...
export const QuizReview = ({
	...
	return (
		<div className="mx-auto flex max-w-7xl flex-col gap-6 pb-24 md:gap-8 lg:grid lg:grid-cols-12 lg:pb-0">
			...
			<div className="md:rounded-5xl order-2 h-fit rounded-3xl border-4 border-white/50 bg-white p-5 shadow-xl md:p-8 lg:order-1 lg:col-span-4">
						...
						return (
							<button
								...
								className={`flex aspect-square w-full grow cursor-pointer items-center justify-center rounded-lg border-2 text-sm font-black transition-all md:rounded-xl md:text-lg ${
									isActive ? "z-10 scale-110 border-slate-800 shadow-lg" : "border-transparent"
			...
			<motion.div
				...
				className="md:rounded-5xl order-1 flex min-h-[400px] flex-col rounded-3xl border-4 border-white/50 bg-white p-6 shadow-xl md:min-h-[500px] md:p-10 lg:order-2 lg:col-span-8"
			>
				...
				<div className="flex items-center justify-between pt-8 md:pt-12">
					...
					<div className="flex gap-2 md:gap-4">
						...
						{currentQuestionIndex < questions.length - 1 && (
							<button
								onClick={onNext}
								className="flex cursor-pointer items-center gap-1 rounded-full bg-slate-800 px-6 py-3 text-sm font-black text-white shadow-lg shadow-slate-800/20 transition-transform hover:scale-105 md:gap-2 md:px-12 md:py-4 md:text-base"
							>
