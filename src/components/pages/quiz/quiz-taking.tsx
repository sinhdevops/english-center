	...
	return (
		<div className="mx-auto flex max-w-7xl flex-col gap-4 pb-20 md:gap-8 lg:grid lg:grid-cols-12 lg:pb-0">
			...
			<motion.div
				...
				className="md:rounded-5xl order-2 h-fit rounded-3xl border-2 border-white/50 bg-white/90 p-5 backdrop-blur-sm md:border-4 md:p-8 lg:order-1 lg:col-span-4"
			>
						...
						return (
							<button
								...
								className={`flex aspect-square w-full grow cursor-pointer items-center justify-center rounded-lg border-2 text-sm font-black transition-all md:rounded-xl md:text-lg ${
									isActive ?"border-stem-blue z-10 scale-110" :"border-transparent"
								} ${isAnswered ?"bg-stem-green text-white" :"bg-white text-black"}`}
							>
								{idx + 1}
							</button>
						);
				...
				<div className="mt-6 space-y-4 border-t border-slate-100 pt-6 md:mt-8">
					...
					<button
						onClick={onFinish}
						className="bg-stem-blue flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl py-3 text-sm font-black text-white transition-all active:scale-95 md:rounded-2xl md:py-4 md:text-base"
					>
			...
			<motion.div
				...
				className="md:rounded-5xl order-1 flex min-h-[350px] flex-col rounded-3xl border-2 border-white/50 bg-[#fdf5e6]/95 p-5 backdrop-blur-sm md:min-h-[500px] md:border-4 md:p-10 lg:order-2 lg:col-span-8"
			>
				...
				<div className="flex items-center justify-between pt-6 md:pt-12">
					...
					{currentQuestionIndex === questions.length - 1 ? (
						<button
							onClick={onFinish}
							className="bg-stem-red flex cursor-pointer items-center gap-2 rounded-full px-6 py-2.5 text-sm font-black text-white transition-all active:scale-95 md:gap-3 md:px-12 md:py-4 md:text-xl"
						>
						...
						<button
							onClick={onNext}
							className="bg-stem-blue flex cursor-pointer items-center gap-2 rounded-full px-6 py-2.5 text-sm font-black text-white transition-all active:scale-95 md:gap-3 md:px-12 md:py-4 md:text-xl"
						>
