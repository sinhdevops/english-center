...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
	...
	return {
		title: event.title,
		description: event.excerpt || event.description || "Góc học tập tại STEMKey",
		openGraph: {
			title: event.title,
...
export default async function StudyCornerDetailPage({ params }: { params: Promise<{ slug: string }> }) {
	const { slug } = await params;
	const supabase = await createClient();
