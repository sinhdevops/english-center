...
const MaterialItem = ({ title, date, desc, img, onClick }: any) => (
	...
	>
		<Link href={`/khoa-hoc/${title}`} className="flex w-full flex-col gap-6 text-center md:flex-row md:text-left">
...
export default function CoursesPage({ onArticleSelect }: { onArticleSelect?: (article: any) => void }) {
	return (
		<div className="min-h-screen bg-white">
			<div className="border-b border-slate-100 bg-slate-50 py-4">
				<div className="mx-auto max-w-7xl px-4">
					<Breadcrumb items={[{ label: "Khoá học", active: true }]} variant="dark" />
				</div>
			</div>
				...
				<div className="flex flex-col gap-16 lg:flex-row">
					<div className="w-full lg:w-2/3">
						<h1 className="mb-10 flex items-center justify-center gap-3 text-3xl font-black text-slate-900 uppercase md:justify-start">
							<TrendingUp className="text-stem-blue" /> Khóa học tại STEMKey
						</h1>
